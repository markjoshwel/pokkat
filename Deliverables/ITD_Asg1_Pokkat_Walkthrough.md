...
Immersive Technology Development (ITD) Assignment 1
...
3. Tap on "Start Game" to enter game screen the start the experience.

    You will be presented with a view of your device's camera, and a gameplay heads-up display (HUD) that shows two bars:
  
    - **Hunger Bar:** (Depicted with a food can icon) Indicates the cat's hunger level.
    - **Happiness Bar:** (Depicted with a heart icon) Indicates the cat's happiness level.

    (add details on time decay)

4. Follow the prompt text:

    - (add bullet points here as per game state and ui prompt text)
    
    (add on how you may interact with the cat, spawning a bowl by touching the screen to place it in AR space, petting via ui, petting via touching the cat on screen, or with spawning friend nekos with a duplicate tracking marker etc)

6. At any point, or once the cat is satiated and you need not continue interacting with the cats, tap on the "Exit" button to leave the game and return to the main menu.
